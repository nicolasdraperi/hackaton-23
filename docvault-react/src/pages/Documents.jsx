...
export function Upload({ adminMode = false, users = [] }) {
  ...
  const submit = async (e) => {
    ...
    try {
      ...
      // Analyse OCR pour chaque fichier uploadé
      const results = [];
      for (const f of files) {
        const res = await api.ocr(f);
        results.push({ name: f.name, ocr: res });
      }
  ...
  return (
    <Page>
      <PageHead
        ...
        sub={
          adminMode
            ? "Uploadez des documents pour n'importe quel utilisateur"
            : "Selectionnez plusieurs fichiers PDF ou images"
        }
      />
            ...
            <form onSubmit={submit}>
              {adminMode && users.length > 0 && (
                <Select
                  ...
                  hint="Laissez vide pour uploader en votre nom."
                >
              ...
              <Field
                label={
                  <>
                    Nom du lot{" "}
                    <span style={{ color: C.ink2, fontWeight: 400 }}>
                      (optionnel)
                    </span>
                  </>
                }
                ...
                <div style={{ padding: "1rem" }}>
                  ...
                  {ocrResults.map((r) => (
                    <div key={r.name}>
                      ...
                      <pre>{JSON.stringify(r.ocr, null, 2)}</pre>
                    </div>
                  ))}
                </div>
              </Card>
  ...
  return (
    <Page>
      <PageHead
        ...
        action={
          <Btn variant="accent" onClick={() => navigate("/upload")}>
            Nouvel upload
          </Btn>
        }
      />
      ...
      {loading ? (
        ...
        <Card style={{ textAlign: "center", padding: "3rem" }}>
          ...
          <p
            style={{ fontSize: ".85rem", color: C.ink2, marginBottom: "1rem" }}
          >
            {status
              ? "Aucun lot avec ce statut."
              : "Vous n'avez pas encore uploade de documents."}
          </p>
          <Btn variant="accent" onClick={() => navigate("/upload")}>
            Uploader maintenant
          </Btn>
...
export function OCRDemo({ file }) {
  ...
  return (
    <div>
      <Btn variant="accent" onClick={handleOCR} disabled={!file || loading}>
        {loading ? "Analyse en cours..." : "Lancer OCR"}
      </Btn>
